  ...
  for (;;) {
    ...
    for (p = proc; p < &proc[NPROC]; p++) 
    {
        ...
        if (p->state == RUNNABLE) 
        {
          ...
          {
            if (selected == 0 || (selected != 0 && p->tickets == selected->tickets && p->arrival_time < selected->arrival_time)) 
            {
              selected = p;  
            }
            break;  
...
void mlfq_scheduler()
{
    ...
    for (;;)
    {
        ...
        // Boost priorities if system ticks exceed boost_ticks
        ...
        // Update ticks and check for preemption
        for (p = proc; p < &proc[NPROC]; p++)
        {
            acquire(&p->lock);
            if (p->state == RUNNABLE)
            {
                // Increment ticks_done and check time slice expiration
                p->ticks_done++;
        ...
        // Preempt if a new high-priority process is available
        if (c->proc != 0 && selected->current_priority < c->proc->current_priority)
        {
            ...
            // Move the preempted process to the next lower queue
...
struct syscall_log *syscall_log = 0;